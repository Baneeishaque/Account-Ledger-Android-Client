                ...
                if (json_array.getJSONObject(0).getString("status").equals("1")) {
                    ...
                    if (v2_flag) {
                        ...
                        for (int i = 1; i < json_array.length(); i++) {
                            ...
                            pass_book_entries_v2.add(new Pass_Book_Entry_v2(mysql_date_time_format.parse(json_array.getJSONObject(i).getString("event_date_time")), json_array.getJSONObject(i).getString("particulars"), json_array.getJSONObject(i).getString("from_account_id").equals(current_account_id) ? json_array.getJSONObject(i).getString("to_account_name"):json_array.getJSONObject(i).getString("from_account_name"), json_array.getJSONObject(i).getString("from_account_id").equals(current_account_id) ? 0 : Double.parseDouble(json_array.getJSONObject(i).getString("amount")), json_array.getJSONObject(i).getString("from_account_id").equals(current_account_id) ? Double.parseDouble(json_array.getJSONObject(i).getString("amount")) : 0, balance));
                        }
