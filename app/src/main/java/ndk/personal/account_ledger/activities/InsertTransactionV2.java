...
public class InsertTransactionV2 extends AppCompatActivity {
  ...
  String currentToAccountIdParent = "0", currentToAccountType,
         currentToAccountCommodityType, currentToAccountCommodityValue;
  String currentFromAccountIdParent, currentFromAccountType,
  ...
  AutoCompleteTextView autoCompleteTextViewToAccount,
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    autoCompleteTextViewFromAccount =
        findViewById(R.id.autoCompleteTextView_from);
    ...
    buttonAddFromAccount.setOnClickListener(
        v -> { bindPreviousFromAccount(); });
  ...
  private void exchangeAccounts() {
    ...
    buttonFromAccount.setText(
        buttonToAccount.getText().toString().replace("To", "From"));
    buttonToAccount.setText(temp.replace("From", "To"));

    temp = autoCompleteTextViewFromAccount.getText().toString();
    autoCompleteTextViewFromAccount.setText(
        autoCompleteTextViewToAccount.getText().toString(), false);
    ...
    HttpApiSelectTask.AsyncResponseJsonArray asyncResponseJsonArray =
      ...
      try {
        ...
        if (!jsonArray.getJSONObject(0).getString("status").equals("1")) {
          ...
          for (int i = 1; i < jsonArray.length(); i++) {
            ...
            accounts.add(new Account(
                jsonArray.getJSONObject(i).getString("account_type"),
                jsonArray.getJSONObject(i).getString("account_id"),
                jsonArray.getJSONObject(i).getString("notes"),
                jsonArray.getJSONObject(i).getString("parent_account_id"),
                jsonArray.getJSONObject(i).getString("owner_id"),
                jsonArray.getJSONObject(i).getString("name"),
                jsonArray.getJSONObject(i).getString("commodity_type"),
                jsonArray.getJSONObject(i).getString("commodity_value"),
                jsonArray.getJSONObject(i).getString("name")));
        ...
        Toast
            ...
            .show();
        Log.d(ApplicationSpecification.APPLICATION_NAME,
  ...
  public boolean onOptionsItemSelected(MenuItem item) {
    ...
    if (id == R.id.menu_item_view_from_account_pass_book) {
      ...
      ActivityUtils14.startActivityForClassWithStringExtras(
          currentActivityContext, InsertTransactionV2Via.class,
          new Pair[] {
              new Pair<>("CURRENT_ACCOUNT_ID", currentFromAccountIdParent),
              new Pair<>("CURRENT_ACCOUNT_FULL_NAME",
                         buttonFromAccount.getText().toString().replace(
              ...
              new Pair<>("CURRENT_ACCOUNT_COMMODITY_VALUE",
                         currentFromAccountCommodityValue),
              new Pair<>("CURRENT_ACCOUNT_TAXABLE", currentFromAccountTaxable),
              new Pair<>("CURRENT_ACCOUNT_PLACE_HOLDER",
                         currentFromAccountPlaceHolder)});

    } else if (id == R.id.menu_item_insert_two_way_transaction) {

      ActivityUtils14.startActivityWithStringExtrasAndFinish(
          currentActivityContext, InsertTransactionV2TwoWay.class,
          new Pair[] {
              new Pair<>("CURRENT_ACCOUNT_ID", currentFromAccountIdParent),
              new Pair<>("CURRENT_ACCOUNT_FULL_NAME",
                         buttonFromAccount.getText().toString().replace(
  ...
  private void attemptInsertTransaction() {
    ...
    if (selectedToAccountId.equals("0")) {
      ...
      ValidationUtils.resetErrors(
          new EditText[] {editTextPurpose, editTextAmount});
      Pair<Boolean, EditText> emptyCheckResult =
          ValidationUtils.emptyCheckEditTextPairs(new Pair[] {
              ...
              new Pair<>(editTextPurpose, "Please Enter Purpose...")});
  ...
  public void executeInsertTransaction() {
    ...
    InsertTransactionV2Utils
        .executeInsertTransactionTaskWithClearingOfEditTextsAndIncrementingOfButtonTextTimeStampForFiveMinutes(
            ...
            Integer.parseInt(selectedToAccountId), editTextPurpose,
            editTextAmount, buttonDate, calendar);
  }
}
